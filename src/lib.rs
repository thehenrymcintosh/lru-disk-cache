...
/// Return an iterator of `(path, size)` of files under `path` sorted by ascending last-accessed
/// time, such that the oldest accessed file is returned first.
fn get_all_files<P: AsRef<Path>>(path: P) -> Box<dyn Iterator<Item = (PathBuf, u64)>> {
    let mut files: Vec<_> = WalkDir::new(path.as_ref())
        ...
        .filter_map(|e| {
            e.ok().and_then(|f| {
                ...
                if f.file_type().is_file() {
                    // Get the last-accessed time, size, and the full path.
                    f.metadata().ok().and_then(|m| {
                        m.accessed()
                            .ok()
                            .map(|atime| (atime, f.path().to_owned(), m.len()))
                    })
                } else {
                    None
                }
        ...
        .collect();
    // Sort by last-accessed-time, so oldest file first.
    files.sort_by_key(|k| k.0);
    Box::new(files.into_iter().map(|(_atime, path, size)| (path, size)))
}
...
impl LruDiskCache {
    ...
    ///
    /// Existing files in `path` will be stored with their last-accessed time from the filesystem
...
mod tests {
    ...
    /// Set the last accessed time of `path` backwards by `seconds` seconds.
    fn set_atime_back<T: AsRef<Path>>(path: T, seconds: usize) {
        let m = fs::metadata(path.as_ref()).unwrap();
        let t = FileTime::from_last_access_time(&m);
    ...
    fn test_existing_file_too_large() {
        ...
        set_atime_back(f.create_file("file1", 10), 10);
        set_atime_back(f.create_file("file2", 10), 5);
    ...
    #[test]
    fn test_existing_files_lru_atime() {
        let f = TestFixture::new();
        // Create files explicitly in the past.
        set_atime_back(f.create_file("file1", 10), 5);
        set_atime_back(f.create_file("file2", 10), 10);
    ...
    fn test_add_get_lru() {
        ...
        // Get rid of the cache, to test that the LRU persists on-disk as atimes.
        // This is hacky, but atime resolution on my mac with HFS+ is only 1 second, so we either
        // need to have a 1 second sleep in the test (boo) or adjust the atimes back a bit so
        // that updating one file to the current time actually works to make it newer.
        set_atime_back(f.tmp().join("file1"), 5);
        set_atime_back(f.tmp().join("file3"), 5);
        {
            let mut c = LruDiskCache::new(f.tmp(), 25).unwrap();
            // Bump file1 again.
            c.get("file1").unwrap();
        }
        // Now check that the on-disk atimes were updated and used.
        {
            let mut c = LruDiskCache::new(f.tmp(), 25).unwrap();
            assert!(c.contains_key("file1"));
